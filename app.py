...
def status():
    """
    Endpoint que retorna el estado general del servidor, cantidad de tareas, completadas y pendientes.
    """
    tasks = load_tasks()
    total = len(tasks)
    completadas = sum(1 for t in tasks if t.get("completed"))
    ...
    return jsonify({
        ...
        "puerto_servidor": request.host.split(':')[1],
        "timestamp": datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    }), 200
